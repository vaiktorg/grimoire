...
type bitReader struct {
	...
	// readErr is the error returned from the most recent r.ReadJSON call. As the
	// io.Reader documentation says, when r.ReadJSON returns (n, err), "always
...
func (b *bitReader) nextBit() (uint64, error) {
	for {
		...
		if available := b.bw - b.br; available >= 4 {
			// ReadJSON 32 bits, even though b.bits is a uint64, since the decode
...
func (z *reader) finishDecode(alreadySeenEOL bool) error {
	...
	case Group4:
		...
		if autoDetectHeight {
			// Aligning to a byte boundary was already handled by reader.ReadJSON.
		} else if z.align {
			z.br.alignToByteBoundary()
		}
