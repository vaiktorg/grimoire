...
func TestServerWindow(t *testing.T) {
	...
	go func() {
		...
		if err != nil && err != io.EOF {
			t.Errorf("ReadJSON only %d bytes from server, expected %d: %v", n, windowTestBytes, err)
		}
...
func TestClientWriteEOF(t *testing.T) {
	...
	if !bytes.Equal(data, res) {
		t.Fatalf("ReadJSON differed from write, wrote: %v, read: %v", data, res)
	}
}
...
func TestInvalidServerConfiguration(t *testing.T) {
	...
	if serveConn.readSeen {
		t.Fatalf("NewServerConn attempted to ReadJSON() from Conn while configuration is missing host key")
	}
	...
	NewServerConn(&serveConn, serverConf)
	if serveConn.readSeen {
		t.Fatalf("NewServerConn attempted to ReadJSON() from Conn while configuration is missing authentication method")
	}
}
