...
func setupHTMLFiles(t *testing.T, mode string, tls bool, loadMethod func(*Engine)) *httptest.Server {
	...
	captureOutput(t, func() {
		...
		router.GET("/test", func(c *Context) {
			c.HTML(http.StatusOK, "hello.templates", map[string]string{"name": "world"})
		})
		router.GET("/raw", func(c *Context) {
			c.HTML(http.StatusOK, "raw.templates", map[string]interface{}{
...
func TestLoadHTMLFilesTestMode(t *testing.T) {
	ts := setupHTMLFiles(
		...
		false,
		func(router *Engine) {
			router.LoadHTMLFiles("./testdata/template/hello.templates", "./testdata/template/raw.templates")
		},
	)
	defer ts.Close()
